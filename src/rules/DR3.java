...
public class DR3 extends DeductionRule {
    ...
    public void applyRule(Cell cell, Grid g) {
        ...
        NakedPairs(cell.getRow(g.getCells()),subject);
        NakedPairs(cell.getCol(g.getCells()),subject);
        NakedPairs(cell.getRow(g.getCells()),subject);
    ...
    public void NakedPairs(Elements e, Subject subject) {
        List<Cell> emptyCells = new ArrayList<>();

        // On récupère les cellules qui ont 2 possibilités
                ...
                if (cell1.getPossibilitys().equals(cell2.getPossibilitys())) {
                    ...
                    for (Cell cell : e.getCells()) {
                        if (cell != cell1 && cell != cell2 && cell.getValue() == -1) {
                            // On retire tous les candidats de la nouvelle paire
                            for (Integer candidate : pairPossibilities) {
                                if (cell.removeValue(candidate)) {
                                    subject.notifyObservers(cell);
                                }
                            }
                        }
                    }
